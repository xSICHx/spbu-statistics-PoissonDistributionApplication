...
private:
    ...
    /// <summary>
    /// —оздаЄт сгруппированные данные (удовлетвор€ют услови€м ’и-квадрат).
    /// </summary>
    /// <param name="th_chied">ѕустой массив дл€ теоретических частот, который будет впоследствии заполнен</param>
    /// <param name="sample_chied">ѕустой массив дл€ частот выборки, который будет впоследствии заполнен</param>
    /// <param name="chied_len">ƒлина частот, будет впоследствии вычислена</param>
    void create_chied_values(double*& th_chied, double*& sample_chied, int& chied_len) {
        ...
        // добавл€ем строку в конец
        th_chied[chied_len - 1] = tmp_th2;
        sample_chied[chied_len - 1] = tmp_sample2;

        // добавл€ем строку в начало
        th_chied[0] = tmp_th;
        sample_chied[0] = tmp_sample;
    }

    /// <summary>
    /// —читает хи квадрат по теоретическим и выборочным частотам.
    /// </summary>
    /// <returns>«начение ’и квадрат</returns>
...
public:
	...
	Chi2Histortam(Distribution& d, PoissonSample& sample) {
		...
		th_freq = d.get_th_prob_array(distr_len); // пока что не частоты, а веро€тности
	...
	/// <summary>
	/// —оздаЄт и сохран€ет в себе теоретические и выборочные частоты от одного распределени€  
	/// </summary>
	/// <param name="d"> –аспределение </param>
	/// <param name="sample"> ¬ыборка </param>
	...
	/// <summary>
	/// —оздаЄт и сохран€ет в себе теоретические и выборочные частоты от разных распределений.
	/// </summary>
	/// <param name="d"> “еоретическое распределение </param>
	/// <param name="sample"> ¬ыборка </param>
	/// <param name="d_sample"> –аспределение дл€ выборки </param>
